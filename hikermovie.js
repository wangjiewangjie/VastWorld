function hikhmrule() {
    ...
    d.push({
        title: '检查更新',
        url: $('#noLoading#').lazyRule(() => {
            return hikerUpdateRules(true, true);
        }),
        col_type: 'flex_button'
    });
    d.push({
        title: '本地 v' + (json.version || '?') + (json.build ? (' (' + json.build + ')') : ''),
...
var HIKER_RULE_DIR = 'hiker://files/rules/xyq/';
var HIKER_JS_URL = 'https://raw.githubusercontent.com/wangjiewangjie/VastWorld/main/hikermovie.js';
var HIKER_JSON_URL = 'https://raw.githubusercontent.com/wangjiewangjie/VastWorld/main/hikermovie.json';

function hikerFetchOpt() {
    return {
        headers: {
...
function hikerRuleUrl(base) {
    return base + '?t=' + Date.now();
}

function hikerFetchRule(url, check) {
    try {
        var content = fetch(url, hikerFetchOpt());
...
function hikerGetLocalMeta() {
    try {
        if (!fileExist(HIKER_RULE_DIR + 'hikermovie.json')) {
            return {};
        }
        return JSON.parse(fetch(HIKER_RULE_DIR + 'hikermovie.json', {}));
    } catch (e) {
        return {};
    }
}

function hikerGetRemoteMeta() {
    try {
        var json = JSON.parse(hikerFetchRule(hikerRuleUrl(HIKER_JSON_URL), function(content) {
...
function hikerNeedsUpdate() {
    if (!fileExist(HIKER_RULE_DIR + 'hikerupdate.txt') || !fileExist(HIKER_RULE_DIR + 'hikermovie.js') || !fileExist(HIKER_RULE_DIR + 'hikermovie.json')) {
        return true;
    }
    var local = hikerGetLocalMeta();
    var remote = hikerGetRemoteMeta();
    ...
    var remoteJs = hikerFetchRule(hikerRuleUrl(HIKER_JS_URL), function(content) {
        return content.search(/lazyRule/) != -1;
    });
    var localJs = fetch(HIKER_RULE_DIR + 'hikermovie.js', {});
...
function hikerUpdateRules(showTip, force) {
    if (force == null) {
        force = false;
    }
    if (!force && !hikerNeedsUpdate()) {
        var local = hikerGetLocalMeta();
        if (showTip) {
            confirm({ title: '已是最新', content: '本地 v' + (local.version || '?') + (local.build ? ' (' + local.build + ')' : '') });
        }
        return 'toast://本地已是最新版本';
    }
    var rulejs = hikerFetchRule(hikerRuleUrl(HIKER_JS_URL), function(content) {
        return content.search(/lazyRule/) != -1;
    });
    var rulejson = hikerFetchRule(hikerRuleUrl(HIKER_JSON_URL), function(content) {
        return content.search(/\"vodhref\"/) != -1;
    });
    var jsOk = rulejs != '';
    var jsonOk = rulejson != '';
    if (jsOk) {
        writeFile(HIKER_RULE_DIR + 'hikermovie.js', rulejs);
    }
    if (jsonOk) {
        writeFile(HIKER_RULE_DIR + 'hikermovie.json', rulejson);
    }
    if (jsOk && jsonOk) {
        writeFile(HIKER_RULE_DIR + 'hikerupdate.txt', Date.now() + '');
        var meta = hikerGetLocalMeta();
        if (showTip) {
            confirm({ title: '更新成功', content: '已更新至 v' + (meta.version || '?') + (meta.build ? ' (' + meta.build + ')' : '') + '，正在刷新页面' });
        }
        refreshPage(false);
        return 'toast://更新完成，已刷新页面';
    }
    if (showTip) {
        confirm({ title: '更新失败', content: 'GitHub 拉取失败，请检查网络能否访问 GitHub' });
    }
    return 'toast://更新失败';
}